...
(use-fixtures :each
              (fn [f]
                  ...
                  ;; 重置保护键为默认值（undo 会在 init 中注册 :krro/undo）
...
(deftest test-undo-redo-flow
         (let [initial (user-data @proj/project)]
              ...
              (cmd/execute-command! :krro.command/record-state)
              (cmd/execute-command! :krro.command/undo)
              (is (= initial (user-data @proj/project)))
              (cmd/execute-command! :krro.command/redo)
...
(deftest test-undo-branch-switching
         ...
         (cmd/execute-command! :krro.command/undo)
         (cmd/execute-command! :krro.command/undo-switch-branch 0)
